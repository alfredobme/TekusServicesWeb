...
export class ProvidersService {
  ...
  deleteProvider(providerId: number): Observable<any> {
    return this.usersService.getToken().pipe(
      switchMap(token => {
        ...
        return this.http.delete(`${this.baseUrl}/${providerId}`, {
          headers: { Authorization: `Bearer ${token}` }
        }).pipe(
          catchError((error) => {
            return throwError(() => error);
  ...
  modifyProvider(providerId: number, data: any): Observable<any> {
    return this.usersService.getToken().pipe(
      switchMap(token => {
        if (!token) {
          this.showError('Authorization failed. Please try again')
        ...
        return this.http.put(`${this.baseUrl}/${providerId}`, data, {
          headers: { Authorization: `Bearer ${token}` }
        }).pipe(
          catchError((error) => {
            return throwError(() => error);
